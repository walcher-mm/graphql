...
func (c *Client) SetReturnCodeRequired() {
	c.returnCode = true
}
...
func (c *Client) Run(ctx context.Context, req *Request, resp interface{}) error {
	...
	if c.returnCode {
		return c.runWithJSONandReturnCode(ctx, req, resp)
	}
...
func (c *Client) runWithJSONandReturnCode(ctx context.Context, req *Request, resp interface{}) error {
	var requestBody bytes.Buffer
	requestBodyObj := struct {
		Query     string                 `json:"query"`
		Variables map[string]interface{} `json:"variables"`
	...
	if res.StatusCode != http.StatusOK {
		var buf bytes.Buffer
		if _, err := io.Copy(&buf, res.Body); err == nil {
			return fmt.Errorf("server returned status code: %v: %s", res.StatusCode, buf.String())
		} else {
			return fmt.Errorf("server returned status code: %v: %s %s", res.StatusCode, buf.String(), err.Error())
		}
	}
